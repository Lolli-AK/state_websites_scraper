...
(`snapshot.py`, `normalize.py`, `noncitizen.py`, `uocava.py`) is shared with them almost
...
behaviour. What that costs is duplication: **`normalize.py`, `noncitizen.py` and `uocava.py`
...
| `{state}/scripts/uocava.py` | the military-and-overseas-voter flag written into `meta.json` |
| `{state}/scripts/scan_uocava.py` | that flag over the working tree or all history |
...
the county repos' files with `county` renamed to `state` (as is
`scan_uocava.py`), and all derive the
...
python scripts/scan_uocava.py --history --csv /tmp/uocava-panel.csv
```

At the state level the flag is thin by construction: one of six Texas pages
and three of six Florida pages match, because a secretary of state puts this
material on a dedicated page that is not among the six tracked types. The
county repos carry `scripts/discover_uocava.py` to find those pages; there is
no equivalent here, since these twelve targets are hand-picked rather than
crawled.
