...
Usage:
    ...
    python scripts/snapshot.py --state FL      # one state (repeatable)
    python scripts/snapshot.py --state FL --page-type results
...
# HTTP/2 is enabled deliberately, not incidentally. Several government sites behind
...
A state run is a couple of dozen requests, so the delay costs
    nothing here; it is kept identical to the county repos so the three
    pipelines stay diffable against each other.
    """
...
# while one is still up we store the interstitial instead of the real page —
...
def process_target(row: dict, fetched_at: str, allow_headless: bool) -> dict:
    ...
    out_dir = SNAPSHOTS / state.lower() / page_type
    ...
    #  - plain fetch failed outright (the Texas Secretary of State 403s a
    #    non-browser client outright but serves a real browser fine).
    ...
    if should_escalate:
        log.info("escalating to headless: %s/%s (%s)", state, page_type, url)
        with _HEADLESS_LOCK:
        ...
        if h_result["ok"]:
            ...
            log.warning("headless failed for %s/%s: %s",
                        state, page_type, h_result["error"])
    ...
    # as "the state took this page down". This happens far more from datacenter
    ...
    log.info("%-12s %-12s %-8s %s [%s bytes] %s",
             state, page_type, render_mode, status, byte_size,
             "(external)" if external else "")
    return {"state": state, "page_type": page_type, "render_mode": render_mode,
...
def git_commit(fetched_at: str, n_targets: int) -> None:
    ...
    # The "." pathspec matters: a bare `add -A` stages the whole repository
    # whatever the cwd, so this state's run would commit the other state's tree.
    run("add", "-A", ".")
...
# main
# --------------------------------------------------------------------------- #
def load_targets(states: list[str] | None, page_type: str | None,
    ...
    with MANIFEST.open(newline="", encoding="utf-8") as fh:
        for row in csv.DictReader(fh):
            if not row.get("url", "").strip():
                continue  # gap row: page type doesn't exist for this state
            if states and row["state"].strip().lower() not in states:
                continue
...
def _target_key(row: dict) -> str:
    return f"{row['state'].strip()}/{row['page_type'].strip()}"


def prune_stale_artifacts() -> int:
    """Delete artifact directories that the manifest no longer claims.

    When a row becomes a gap (or a state/page type is removed), its old
    ...
    what makes a state's tree readable at a glance, and it's asserted by
    ...
    for page_dir in sorted(SNAPSHOTS.glob("*/*")):
        ...
        if (page_dir.parent.name, page_dir.name) not in wanted:
            ...
            removed += 1
    # Drop state directories left empty by the above.
    for state_dir in sorted(SNAPSHOTS.glob("*")):
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Snapshot state election pages.")
    ap.add_argument("--state", action="append", default=None,
                    help="limit to state (repeatable, case-insensitive)")
    ...
    # Only prune on a full run: a filtered run legitimately leaves other states'
    # artifacts in place.
    if not (states or args.page_type or args.batch):
        n = prune_stale_artifacts()
        if n:
            log.info("pruned %d artifact dirs no longer in the manifest", n)
    targets = load_targets(states, args.page_type, args.batch)
    ...
    def run_one(row: dict) -> None:
        try:
            ...
            log.exception("unhandled error for %s/%s: %s",
                          row.get("state"), row.get("page_type"), exc)
